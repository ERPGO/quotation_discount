...
class AccountInvoice(models.Model):
    ...
    def _compute_amount( self ):
        ...
        self.total_before_discount = self.amount_untaxed + self.amount_discount
    
    total_before_discount = fields.Monetary(string='Total Amount', store=True, readonly=True, compute='_compute_amount',
    ...
    def supply_rate( self ):
        for inv in self:
            if inv.discount_type == 'percent':
                if inv.discount_rate > 100 or inv.discount_rate < 0:
                    raise ValidationError(_('Enter Discount percentage rate between 0-100.'))
                for line in inv.invoice_line_ids:
                    line.discount = inv.discount_rate
            elif not inv.discount_type:
                inv.discount_rate = 0
                for line in inv.invoice_line_ids:
                    line.discount = inv.discount_rate
            else:
